...
const { activeList, fetchList, updateList, deleteList, duplicateList, fetchLists } =
  useShoppingLists()

const checkedCount = computed(() => activeList.value?.items.filter((i) => i.checked).length ?? 0)
...
<template>
  <div v-if="activeList" class="mx-auto max-w-3xl space-y-6">
    <ListHeader :list="activeList" @rename="onRename" @duplicate="onDuplicate" @delete="onDelete" />

    <TagChips :tags="activeList.tags" @update-tags="onUpdateTags" />

    <ProgressBar :total="activeList.items.length" :checked="checkedCount" />
  ...
  <div v-else class="flex h-full items-center justify-center text-muted-foreground">Loading...</div>
</template>
